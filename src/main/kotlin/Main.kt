...
fun main() {
    convertNcm(Paths.get(System.getProperty("user.home"), "Desktop", "ずっと真夜中でいいのに。 - 脳裏上のクラッカー.ncm").toFile())
}

fun convertNcm(ncmFile: File) {
    ncmFile.inputStream().use { fis ->
        ...
        val musicDataRc4Key = ByteArray(keyLength)
            ...
            .let {
                Cipher.getInstance("AES/ECB/PKCS5Padding")
                    .apply { init(Cipher.DECRYPT_MODE, SecretKeySpec(Constants.RC4_KEY, "AES")) }
                    .doFinal(it)
            }
            .let { String(it) }
        ...
        val metaInfo = ByteArray(metaLength)
            ...
            .let {
                Cipher.getInstance("AES/ECB/PKCS5Padding")
                    .apply { init(Cipher.DECRYPT_MODE, SecretKeySpec(Constants.META_KEY, "AES")) }
                    .doFinal(it)
            }
        ...
        val outputFile = ncmFile.resolveSibling("${ncmFile.nameWithoutExtension}.${metaInfo.format}")
...
fun resolveAlbumImageMimeType(albumImage: ByteArray): String {
